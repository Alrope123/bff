...
fn process_file(
    ...
    let inner_reader: Box<dyn Read> = if reader_mode == 0 {
        Box::new(MultiGzDecoder::new(input_file))
    } else if reader_mode == 1 {
        Box::new(Decoder::new(input_file)?)
    } else {
        Box::new(input_file)
    };
    ...
    for line in reader.lines() {
        ...
        if annotate_attribute_only || annotate_only {
            ...
            data["length"] = serde_json::to_value(Value::String(&text).len()).unwrap();
        } else {
...
fn main() {
    ...
    println!("Suggested filter size: {}", suggested_size);
